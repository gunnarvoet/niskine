...
# %% [markdown]
# # NISKINe Plot Surface Time Series v4

# %% [markdown]
# Change from v3:
# - Plot $\mathrm{NI}_\mathrm{KE}$ on a log-scale.
...
filename = niskine.io.CFG.path.data.joinpath("gridded/temperature/t_daily.nc")
if filename.exists():
    td = xr.open_dataarray(filename)
    td.close()
else:
    td = tall.resample(time="1D").mean()
    # save subsampled version
    td.to_netcdf(filename)
...
# %%
# load NI EKE no WKB
ni_eke_no_wkb = xr.open_dataarray(cfg.data.ni_eke_m1_no_wkb)
ni_eke_no_wkb.close()
...
# %%
fig, ax = gv.plot.quickfig(fgs=(10, 3))

h = np.log10(ni_eke).plot.contourf(
    ax=ax,
    extend="both",
    levels=np.arange(-1.2, 1.0, 0.2, ),
    cmap="Spectral_r",
    cbar_kwargs=dict(
        aspect=30,
        shrink=0.7,
        pad=0.02,
        label="log$_{10}$(NI$_\mathrm{KE}$) [J m$^{-3}$]",
        ticks=np.arange(-1.0, 1.0, 0.4),
    ),
)
ax.invert_yaxis()
ax.set(xlabel=None)
niskine.io.png("NI_KE_log_scale")

# %%
fig, ax = gv.plot.quickfig(fgs=(10, 3))

h = np.log10(ni_eke_no_wkb).plot.contourf(
    ax=ax,
    extend="both",
    levels=np.arange(-1.2, 1.0, 0.2, ),
    cmap="Spectral_r",
...
ax.invert_yaxis()
ax.set(xlabel=None)
niskine.io.png("NI_KE_log_scale_no_WKB")
...
h = np.log10(ni_eke_no_wkb).plot.contourf(
    ax=ax,
    extend="both",
    levels=np.arange(-2.0, 1.25, 0.25, ),
    cmap="Spectral_r",
    cbar_kwargs=dict(
        aspect=30,
        shrink=0.7,
...
ax.annotate(
    "log$_\mathrm{10}$(NI KE)", xy=(0.95, 0.05), xycoords="axes fraction", backgroundcolor="w", ha="right"
)
